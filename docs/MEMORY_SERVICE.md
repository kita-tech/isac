...
`metadata` フィールドは自由形式の dict だが、一部の type では Memory Service が
保存時に **自動補完・検証** を行う。スキーマ定義は `main.py` の `TYPE_METADATA_SCHEMA`
が単一の真実とする（ドキュメントと実装の二重管理を避けるため、変更時は両方更新すること）。

| type | キー | 必須 | 既定値 | 取りうる値 | 補完・検証の挙動 |
|------|------|------|--------|-----------|----------------|
| todo | `owner` | ✓ | （自動補完） | 文字列（メール等） | 未指定時は認証principal（匿名・管理者を除く）から自動補完。補完できない場合は **400で弾かず**レスポンスの `warnings` に追記して保存する（後方互換維持） |
| todo | `status` | ✓ | `pending` | `pending` / `done` | 未指定時は `pending` を補完。`pending`/`done` 以外は **422エラー** |

**設計方針**:
- **owner は必須化（400）しない**。owner の無い todo は `/my/todos` の owner フィルタに現れないだけで、データ自体は有効。安全側に倒し、警告で気付けるようにする。
- owner を確実に設定したい場合は **生API直叩きではなく `/isac-todo`（`isac-todo add`）スキルを使う**こと。スキルが `git config user.email` を owner に設定する。
- enum 違反（status の不正値など）は明確な誤りなので 422 で弾く。

**`warnings` フィールド**（`POST /store` のレスポンス）:

```json
{
  "id": "abc123",
  "message": "Memory stored (project/todo)",
  "warnings": [
    "type=todo に metadata.owner が未指定です。owner の無い todo は /my/todos の owner フィルタに現れません。isac-todo スキル経由での登録、または metadata.owner の明示指定を推奨します。"
  ]
}
```
...
`POST /store` は保存前に、`content` / `summary` / `metadata` の文字列値から
**制御文字（タブ・改行・復帰を除く U+0000–U+001F と DEL U+007F）を除去** する。
ANSI エスケープシーケンスや CLI のページトークン等が保存され、後段の JSON 処理を
壊すのを防ぐための堅牢化措置（`sanitize_control_chars` / `sanitize_metadata`）。

> **補足（クライアント側の堅牢化）**: シェルで JSON を扱う際、`echo "$VAR" | jq` は
> **zsh の `echo` がエスケープ列 `\n` を実際の改行に変換**して JSON を壊すため使用しない。
> 詳細は CLAUDE.md「シェルでの JSON 処理（堅牢化ルール）」を参照。
