...
├── hooks/
├── personas/              # ペルソナ定義ファイル
│   ├── default.md         # デフォルト（ペルソナなし）
│   └── king-bonbi.md      # キングボンビー
└── skills/
```
...
isac persona list       # 利用可能なペルソナ一覧
isac persona set <name> # ペルソナを設定（.isac.yaml を更新）
isac persona show       # 現在のペルソナを表示
```
...
- ペルソナ定義ファイルは `~/.isac/personas/` に配置し、`isac init`/`isac switch` 時に CLAUDE.md の `<!-- ISAC:PERSONA:BEGIN -->` / `<!-- ISAC:PERSONA:END -->` マーカー間を更新する
- ペルソナの優先順位: `.isac.yaml` の `persona` > `~/.isac/config.yaml` の `default_persona` > デフォルト（ペルソナなし）
- ペルソナ名にパストラバーサル文字（`/`, `..`）を含めることは禁止（セキュリティ対策）
