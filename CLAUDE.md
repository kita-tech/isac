...
│   ├── skills/            # Skill 定義
│   ├── agents/            # サブエージェント定義
│   └── workflows/         # マルチエージェント Workflow スクリプト（例: isac-review-agents.js）
...
├── skills/
├── agents/
└── workflows/             # 配布された Workflow スクリプト
```
...
| `/isac-review` | 設計レビュー（ペルソナ議論。`--agents` でブラインド並列エージェントによる同調バイアス低減モード） |
...
`high` 以上、CRUD・整形系は `low`）。

### Skill ごとの model は原則 inherit（指定しない）

`model` フィールドは**原則として指定せず、セッションのモデルを継承（inherit）する**。
スキルの軽重は `effort`（環境非依存の抽象レベル）で表現し、どのモデルで動かすかは
実行環境（各自のセッション）に委ねる。`isac-review`（4ペルソナ）で全員一致した方針。

| 観点 | model 固定（しない理由） |
|------|------------------------|
| 陳腐化メンテ | model ID（例 `claude-opus-4-8`）は具体名のため、新モデル世代ごとに全 SKILL.md の置換が必要。`effort` の抽象レベルと違い必ず腐る |
| チーム配布の事故 | メンバーの契約プランで使えるモデルが異なる。`model:` 固定は対応外モデルの環境でスキルを壊す。`inherit` なら各自が選べる範囲で安全に動く |
| 二重管理 | 「軽い→安い／重い→賢い」の意図は `effort` で表現済み。`model` を重ねると責務が重複し矛盾しうる（`effort: low` なのに `model: opus` 等） |
| YAGNI | 実害（コスト超過／品質不足）が観測されていない。軽量 CRUD 系を haiku に下げても削減額は誤差で、`isac-todo` の機密フィルタ判断などを下位モデルに任せるリスクが割高 |

**例外**: `model` 指定が正当化されるのは「特定モデルでないと壊れる技術的必然がある」または
「実測でコスト/品質の問題が観測された」場合のみ。それ以外は `model` を書かない。
...
ペルソナレビュー（`/isac-review` 等）の**4人は全員同じモデル（Claude）であり、知識の盲点が相関する**。
そのため、ペルソナ合意で保証できるのは「トレードオフ・考慮漏れ・設計判断」までで、**事実の正しさ（correctness）は保証できない**。
同一モデルの自己レビューは、見落としよりも「**そもそも誤って覚えている事実（plausible-but-wrong）**」に原理的に弱い。

| 領域 | 潰し方 |
|------|--------|
| 設計判断・トレードオフ・考慮漏れ | ペルソナ議論（`/isac-review` 等） |
| 事実の正しさ・機械的バグ（ツール/コマンド/ランタイムの挙動、シェル、SQL、正規表現） | **実行検証**（`bash -n`／stub 実走／`docker` で環境依存確認／`/verify` スキル） |
| 重要設計の最終確認 | 可能なら**独立した別モデル**でクロスチェック（盲点が相関しないため最も効く。CLI が無ければ省略） |

**鉄則**:
- **設計書に runnable なコードを載せるときは、確定前に必ず動かす**。議論だけで「正しい」と確定しない。
- 検証した主張は ✅検証済、未検証は ⚠️未検証 とラベルする。**「もっともらしい補足」で考えてある感を出して誤りを覆い隠さない**。
- 単一モデルの自己レビューを「最終保証」扱いしない。
