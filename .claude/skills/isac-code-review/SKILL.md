...
| 懐疑的レビュアー | 批判的検証 | 要件との乖離、過剰実装、見落とされたedge case、より単純な代替案の有無、**もっともらしいが誤った事実主張（plausible-but-wrong）** |

> **注**: 懐疑的レビュアーはメタレビューとして機能し、スコアリングの重み付けには含まれません。指摘は「改善提案」として出力されます。
>
> **必須ルール**: 懐疑的レビュアーは常に含めること（ペルソナ数に関わらず最低1人）。
>
> **同一モデルの限界**: 4ペルソナは全員同じモデルで盲点が相関する。合意は「設計判断・考慮漏れ」には有効だが
> **事実の正しさ（correctness）を保証しない**。ツール/コマンド/ランタイム挙動の断定は実行で確認すること（下記「実行検証」）。

### 実行検証（runnable なコードを指摘・提案する場合）

レビューで**実行可能なコード/コマンドの正しさ**を断定する場合、または修正案として runnable な成果物を提示する場合は、
**可能な範囲で実際に動かして確認する**こと（`bash -n`／最小ケース実行／`docker` で環境依存挙動の確認）。
検証した指摘は ✅検証済、未検証の事実主張は ⚠️未検証 とラベルし、**未検証を「確定」として提示しない**。
組み込みの `/verify` スキルが使える場合は活用する。検証手段が無い場合はその旨を明記する。
