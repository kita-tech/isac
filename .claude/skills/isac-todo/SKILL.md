...
INDEX=$((TARGET_NUM - 1))
MEMORY_ID=$(printf '%s\n' "$RESULT" | jq -r ".todos[$INDEX].id")
TASK_CONTENT=$(printf '%s\n' "$RESULT" | jq -r ".todos[$INDEX].content | split(\"\n\")[0] | .[0:60]")
...
if printf '%s\n' "$RESPONSE" | jq -e '.id' > /dev/null 2>&1; then
    ...
    # NOTE: 一旦変数に格納し printf '%s\n' で渡す（curl | jq 直結はページャ等で制御文字混入の懸念があるため避ける）
    REMAINING_JSON=$(curl -s "$MEMORY_URL/my/todos?project_id=$PROJECT_ID&owner=$USER_EMAIL&status=pending")
    REMAINING=$(printf '%s\n' "$REMAINING_JSON" | jq -r '.count')
    echo "✅ タスクを完了しました"
    echo ""
    echo "「$TASK_CONTENT」"
    echo ""
...
echo ""
printf '%s\n' "$RESULT" | jq -r '.todos | to_entries | .[] | "  \(.key + 1). [x] \(.value.content | split("\n")[0] | .[0:60]) (ID: \(.value.id))"'
echo ""
