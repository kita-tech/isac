---
name: isac-autopilot
description: 要件からコンテキスト収集・設計・実装・テスト・レビュー・Draft PR作成までを自動実行します。
---

# ISAC Autopilot Skill

要件を入力すると、コンテキスト収集→設計→実装→テスト→4ペルソナレビュー→Draft PR作成までを自動実行します。

## 使い方

```
/isac-autopilot <要件>
```

## Phase 構成

```
Phase 0: コンテキスト収集（実装エージェント）
Phase 1: 設計（実装エージェント）
Phase 1.5: Worktree 準備（実装エージェント）
  → 作成失敗時は従来動作にフォールバック
Phase 2: 実装（実装エージェント）※ worktree 内
Phase 3: テスト実行（実装エージェント）※ worktree 内
Phase 4: 4ペルソナレビュー（別エージェント）
  → 不合格（90点未満 or 🔴指摘あり）: Phase 2 に戻る（最大2回ループ、worktree は維持）
  → ループ時も Phase 2→3→4 の順で再実行
Phase 5: Draft PR作成 + Worktree クリーンアップ（実装エージェント）
  → Phase 4 のレビュー結果を gh pr comment で投稿
```

## 重要: 実行方法

**このスキルはフォアグラウンドで自動実行されます。**

Claude は以下の手順で実行してください：

1. **Task tool を起動**（`run_in_background` は指定しない）
2. **ユーザーには「Autopilot を実行中」と伝える**
3. **完了後、結果（Draft PR URL等）をユーザーに報告**

### 実行コード例

```
Task tool を以下のパラメータで呼び出す:
- subagent_type: "general-purpose"
- prompt: [下記の完全なプロンプトを渡す]
```

**注意**: フォアグラウンド実行にすることで、MCP ツール（notion, context7 等）がサブエージェント内で利用可能になります。

## エージェントへのプロンプト

以下のプロンプトをそのまま Task tool に渡してください（{requirement} を実際の要件で置換）：

---

あなたは ISAC Autopilot の実装エージェント（オーケストレーター）です。以下の要件を完全に自動で実装してください。
**ユーザーへの質問や承認の要求は一切せず、自律的に判断して進めてください。**

### 作業ディレクトリ

Phase 1.5 で git worktree を作成し `cd` で移動する。
以降の Phase 2〜5 では **カレントディレクトリが worktree** であることを前提に作業する。

- Bash コマンド: そのまま実行（`cd` 後なので worktree 内で動作）
- Read/Edit/Write tool: `pwd` の結果を使って絶対パスを構築
  - 例: `${PWD}/memory-service/main.py`（`pwd` を Bash で取得してから使う）
- worktree 作成に失敗した場合: 元ディレクトリで従来通り作業する

## 要件

{requirement}

## 実行フロー

### Phase 0: コンテキスト収集（自動）

プロジェクトルールの読み込みと Memory Service からの記憶取得を行う。

**重要**: サブエージェントは CLAUDE.md を自動的に継承しないため、最初に明示的に読み込む必要がある。

1. **CLAUDE.md を読み込む**（最優先）:
   - Read tool で `CLAUDE.md` を読み込み、記載されたすべてのルールに従うこと
   - サブエージェントは CLAUDE.md を自動継承しないため、この明示的な読み込みが必須

2. **project_id を取得**:
```bash
PROJECT_ID=$(grep "project_id:" .isac.yaml 2>/dev/null | sed 's/project_id: *//' | tr -d '"'"'" || echo "${CLAUDE_PROJECT:-default}")
MEMORY_URL="${MEMORY_SERVICE_URL:-http://localhost:8100}"
ENCODED_PROJECT_ID=$(printf '%s' "$PROJECT_ID" | jq -sRr @uri 2>/dev/null || printf '%s' "$PROJECT_ID")
```

3. **グローバルルールを常時取得**（要件に関係なく必ず実行）:
```bash
GLOBAL_RULES=$(curl -s --max-time 3 \
    "$MEMORY_URL/search?scope=global&type=decision&limit=10" \
    2>/dev/null || echo "")
```
グローバルルール（PR作成時のドキュメント更新確認、コーディング規約等）は要件と無関係に常に適用されるため、クエリなしで取得する。

4. **要件ベースのプロジェクト関連記憶を取得**:
要件から主要なキーワードを2-3語抽出し、スペース区切りで検索クエリとして使用する。
```bash
CONTEXT=$(curl -s --max-time 5 "$MEMORY_URL/context/$ENCODED_PROJECT_ID" \
    --get \
    --data-urlencode "query={要件から抽出したキーワード 2-3語}" \
    --data-urlencode "max_tokens=2000" \
    2>/dev/null || echo "")
```

5. **取得した記憶を以降の Phase で活用**:
   - ステップ3の `memories`: グローバルルール（ワークフロー、ドキュメント更新ルール等）
   - ステップ4の `global_knowledge`: 要件に関連するグローバルナレッジ
   - ステップ4の `project_decisions`: プロジェクト固有の決定事項
   - ステップ4の `project_recent`: 最近の関連作業

6. **環境確認**:
   - Memory Service の接続状況（`curl -s --connect-timeout 1 "$MEMORY_URL/health"` で確認）
   - 利用可能な MCP ツールの一覧を出力（フォアグラウンド実行なので親プロセスの MCP ツールが継承される）

7. 出力: CLAUDE.md のルール確認結果、接続状況、取得した記憶の要約（何件取得したか、主要なルール）

**重要**: Memory Service が応答しない場合はスキップして Phase 1 に進む（エラーで停止しない）。MCP ツールがない場合も警告を出力して続行する。

### Phase 1: 設計（自動）

Phase 0 で取得した記憶を考慮しながら、以下の3ステップで設計を行う。

#### Step 1: 影響範囲の特定

1. 要件を分析し、変更が必要なファイルを Glob/Grep/Read で探索
2. 各ファイルの変更内容（追加/修正/削除）を一覧化
3. 依存関係（このファイルを変更すると影響するファイル）も特定
4. 出力: 影響範囲テーブル

#### Step 2: テストシナリオ作成

1. 実装予定の変更に対するテストシナリオを作成
2. 正常系・異常系・境界値・エッジケースを含める
3. テストシナリオは Phase 2 の実装時に参照し、テストコード作成に使用
4. 出力: テストシナリオ一覧

#### Step 3: 4ペルソナ設計レビュー

Step 1 の影響範囲・実装方針と Step 2 のテストシナリオに対して、4つのペルソナで設計レビューを実施する。
**別エージェントは起動せず、実装エージェント自身が4つの視点で検討する。**

| ペルソナ | 視点 |
|---------|------|
| 実装者 | 実装の容易さ、保守性 |
| 運用者 | 運用負荷、スケーラビリティ |
| アーキテクト | 全体設計、拡張性、技術的負債 |
| 懐疑的レビュアー | 要件との乖離、過剰設計、テストシナリオの網羅性、より単純な代替案 |

各ペルソナの指摘を検討し、設計方針やテストシナリオに問題があれば修正する。

#### Phase 1 の出力

以下のフォーマットで出力する：

```
### 影響範囲

| ファイル | 変更種別 | 変更内容 | 影響 |
|---------|---------|---------|------|
| path/to/file | 追加/修正/削除 | 概要 | 依存先への影響 |

### テストシナリオ

| # | シナリオ | 種別 | 期待結果 |
|---|---------|------|---------|
| 1 | ... | 正常系 | ... |
| 2 | ... | 異常系 | ... |
| 3 | ... | 境界値 | ... |
| 4 | ... | エッジケース | ... |

### 設計レビュー結果

| ペルソナ | 判定 | コメント |
|---------|------|---------|
| 実装者 | ✅/⚠️ | {コメント} |
| 運用者 | ✅/⚠️ | {コメント} |
| アーキテクト | ✅/⚠️ | {コメント} |
| 懐疑的レビュアー | ✅/⚠️ | {コメント} |

結論: {設計方針の最終判断}
```

### Phase 1.5: Worktree 準備（自動）

メインの作業ディレクトリを保護するため git worktree を作成し、以降の作業を分離する。

#### 手順

1. **変数を設定**:
```bash
ORIGINAL_DIR="$(pwd)"
REPO_NAME="$(basename "$ORIGINAL_DIR")"
SHORT_DESC="{要件から3-5単語のケバブケース}"
BRANCH_NAME="autopilot/$(date +%Y%m%d)-${SHORT_DESC}"
WORKTREE_DIR="${ORIGINAL_DIR}/../${REPO_NAME}-autopilot-${SHORT_DESC}"
```

2. **worktree を作成**:
```bash
git worktree add -b "$BRANCH_NAME" "$WORKTREE_DIR" HEAD
```

3. **gitignore 対象ファイルを準備**:
```bash
# シークレット設定のコピー
[ -f "$ORIGINAL_DIR/.isac.secrets.yaml" ] && cp "$ORIGINAL_DIR/.isac.secrets.yaml" "$WORKTREE_DIR/.isac.secrets.yaml" && chmod 600 "$WORKTREE_DIR/.isac.secrets.yaml"

# Python 仮想環境のシンボリックリンク
[ -d "$ORIGINAL_DIR/.venv" ] && ln -s "$ORIGINAL_DIR/.venv" "$WORKTREE_DIR/.venv"
```

4. **カレントディレクトリを worktree に切り替え**:
```bash
cd "$WORKTREE_DIR"
pwd  # 確認用に出力
```

**以降の Phase 2〜5 は、このカレントディレクトリ（worktree）内で実行する。**

#### エラーハンドリング（フォールバック）

worktree 作成に失敗した場合（ディレクトリ既存、ブランチ名衝突等）:

1. 既存 worktree を検出した場合: `git worktree remove "$WORKTREE_DIR" --force` してリトライ
2. ブランチ名衝突: サフィックス `-2` を付与してリトライ
3. **それでも失敗: 元ディレクトリで従来通り作業を続行**（Phase 5 でブランチ作成）

フォールバック時は以下を出力:
```
⚠️ Worktree の作成に失敗しました。元ディレクトリで作業を続行します。
   理由: {エラーメッセージ}
```

#### 出力

```
🌿 Phase 1.5: Worktree 準備
  ✓ ブランチ: {BRANCH_NAME}
  ✓ Worktree: {WORKTREE_DIR}
  ✓ カレントディレクトリ: {pwd の結果}
```

### Phase 2: 実装（自動）

**カレントディレクトリ（worktree）内で作業すること。**
ファイルの Read/Edit/Write には、Bash の `pwd` で取得した絶対パスを使用する。

1. コードを実装（Edit/Write tool使用）
2. 必要に応じてテストコードも作成
3. 出力: 変更ファイル一覧

### Phase 3: テスト実行（自動）

**カレントディレクトリ（worktree）内でテストを実行する。**

1. テストコマンドを自動検出（pytest, npm test, go test など）
2. テスト実行
   - `source .venv/bin/activate && pytest ...` はシンボリックリンク経由で動作する
   - `bash tests/run_all_tests.sh` は `SCRIPT_DIR` からパスを自動解決するため、そのまま動作する
3. 失敗時: エラー内容を記録し、Phase 2 のループ判定に含める

### Phase 4: 4ペルソナレビュー（別エージェント）

**重要**: このPhaseでは Task tool を使って別のサブエージェント（general-purpose）を起動し、レビューを実行する。実装エージェント自身がレビューするのではなく、独立したエージェントに委託することでバイアスを排除する。

#### レビューエージェントの起動方法

Task tool を以下のパラメータで呼び出す:
- subagent_type: "general-purpose"
- prompt: 下記のレビュープロンプト（{変数} を実際の値で置換）

#### レビューエージェントへのプロンプト

```
あなたは ISAC Autopilot のレビューエージェントです。以下の変更内容を4つのペルソナ + PR固有のチェック項目でレビューしてください。

## 要件

{要件の説明}

## プロジェクトルール

Read tool で CLAUDE.md を読み込み、プロジェクトのルール・規約を把握してください。

## レビュー対象コンテキスト

### 変更ファイル一覧

{カレントディレクトリ（worktree）で `git diff HEAD --name-status` を実行した結果}

### 変更内容（git diff）

{カレントディレクトリ（worktree）で `git diff HEAD` を実行した結果}

### 関連ファイル参照

変更ファイルが参照・依存しているファイル（import先、設定ファイル、スキル定義等）は Read tool で読み込み、整合性を確認してください。

## レビュー手順

### 評価観点と重み付け

| 観点 | 重み | 評価内容 |
|------|------|----------|
| コード品質 | 30% | 保守性、可読性、DRY/SOLID原則 |
| セキュリティ | 25% | 脆弱性、認証情報の誤コミット、データ保護 |
| テスト充足度 | 20% | テストファイルの追加・変更有無 |
| PR品質 | 15% | サイズ、コミット粒度、変更の説明性 |
| 整合性 | 10% | ファイル間の整合性、変更の一貫性 |

### PR固有のチェック項目

#### セキュリティ（機密情報検出）
- [ ] `.env` ファイルの誤コミット
- [ ] APIキー、パスワード、トークンのハードコード
- [ ] `credentials`, `secrets` を含むファイル
- [ ] 秘密鍵（`.pem`, `.key`）

#### テスト充足度
- [ ] 機能追加に対するテストの有無
- [ ] テストファイルの変更有無
- [ ] テストカバレッジへの影響

#### PR品質
- [ ] PRサイズ（推奨: 400行以下）
- [ ] コミット粒度（1コミット1目的）
- [ ] コミットメッセージの品質

#### データベース（クエリ変更がある場合）
- [ ] 新規クエリがインデックスを活用しているか
- [ ] WHERE句・JOIN条件のカラムにインデックスが存在するか
- [ ] フルテーブルスキャンを引き起こすクエリがないか
- [ ] ORDER BY / GROUP BY がインデックスを活用しているか
- [ ] 新規テーブル・カラム追加時にインデックス定義が適切か

#### 整合性
- [ ] インポート/エクスポートの整合性
- [ ] 型定義の一貫性
- [ ] 命名規則の統一

### ペルソナ一覧

以下の4ペルソナで独立した視点で評価すること。

| ペルソナ | 専門 | チェック観点 |
|---------|------|-------------|
| セキュリティ専門家 | 脆弱性診断 | 機密情報漏洩、インジェクション、認証・認可、入力検証、OWASP Top 10 |
| パフォーマンス専門家 | 最適化 | N+1問題、計算量、メモリ効率、キャッシュ、非同期処理、DBインデックス活用 |
| 品質・保守性専門家 | コード品質 | 可読性、命名、DRY/SOLID、テスト容易性、エラーハンドリング |
| 懐疑的レビュアー | 批判的検証 | 要件との乖離、過剰実装、見落とされたedge case、より単純な代替案の有無 |

> **注**: 懐疑的レビュアーはメタレビューとして機能し、スコアリングの重み付けには含まれません。指摘は「改善提案」として出力されます。
>
> **必須ルール**: 懐疑的レビュアーは常に含めること（ペルソナ数に関わらず最低1人）。

### スコアリング

各観点を100点満点で評価し、重みを掛けて合計100点満点とする。

計算方法:
- コード品質: XX/100 × 0.30 = XX
- セキュリティ: XX/100 × 0.25 = XX
- テスト充足度: XX/100 × 0.20 = XX
- PR品質: XX/100 × 0.15 = XX
- 整合性: XX/100 × 0.10 = XX
- 合計: XX/100

### 出力フォーマット（必ずこの形式で出力）

最初の2行に以下の形式でスコアとクリティカル指摘数を出力（パース用）:
REVIEW_SCORE: XX/100
CRITICAL_COUNT: X

次に詳細レビューを出力:

## 観点別スコア

| 観点 | スコア | 重み | 寄与 |
|------|--------|------|------|
| コード品質 | XX | 30% | XX |
| セキュリティ | XX | 25% | XX |
| テスト充足度 | XX | 20% | XX |
| PR品質 | XX | 15% | XX |
| 整合性 | XX | 10% | XX |

**合計: XX/100**

## 詳細レビュー

### セキュリティ専門家

| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🔴/🟡/🟢 | `path/to/file` | {内容} |

### パフォーマンス専門家

| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🔴/🟡/🟢 | `path/to/file` | {内容} |

### 品質・保守性専門家

| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🔴/🟡/🟢 | `path/to/file` | {内容} |

### 懐疑的レビュアー

| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🔴/🟡/🟢 | `path/to/file` | {内容} |

## 改善提案（+XX）

1. **filename: 指摘タイトル** (+X)
   - 現状: ...
   - 改善案: ...

## PR品質チェック

### 基本
- ✅/⚠️ PRサイズ: {評価}（XXX行）
- ✅/⚠️ コミット粒度: {評価}
- ✅/⚠️ テスト: {評価}

### 整合性
- ✅/⚠️ インポート/エクスポート: {評価}
- ✅/⚠️ 型定義の一貫性: {評価}
- ✅/⚠️ 命名規則: {評価}

## 総合判定

- 合格（90点以上 かつ 🔴指摘が0件）/ 不合格（90点未満 または 🔴指摘あり）
- 不合格の場合、修正すべき項目を優先順位付きで列挙

**全て改善した場合の予想スコア: XX/100**
```

#### 判定と分岐

レビューエージェントの出力から `REVIEW_SCORE: XX/100` と `CRITICAL_COUNT: X` を抽出する。

- **合格**（90点以上 かつ 🔴指摘が0件）→ Phase 5 へ
- **不合格**（90点未満 または 🔴指摘あり）→ Phase 2 に戻る（最大2回ループ）
  - ループ時はレビューの指摘事項（特に🔴指摘を優先）を修正し、Phase 2→3→4 の順で再実行
  - 3回目のレビュー（2回ループ後）でも不合格の場合は、現状のまま Phase 5 に進み、人間に引き継ぐ

#### ループ経緯の記録

各ループのイテレーション情報を記録する：

```
Iteration 1: XX/100
  指摘: {主な指摘内容}
  修正: {修正内容}
Iteration 2: XX/100
  指摘: {主な指摘内容}
  修正: {修正内容}
Iteration 3: XX/100（最終）
```

### Phase 5: Draft PR作成 + Worktree クリーンアップ（自動）

PR作成は実装エージェント（オーケストレーター）自身が行う。

1. **ブランチは Phase 1.5 で作成済み**（フォールバック時はここで作成: `git checkout -b "autopilot/..."`）
2. コミット作成（Co-Authored-By 付き）
3. リモートに push: `git push -u origin "$(git branch --show-current)"`
4. Draft PR作成: `gh pr create --draft`
5. **PR本文に設計ドキュメントとテスト結果を含める**（Phase 1の設計内容を記載）
6. **Phase 4 のレビュー結果を `gh pr comment` で投稿**
7. **Worktree クリーンアップ**（worktree 使用時のみ）:
   ```bash
   cd "$ORIGINAL_DIR"
   git worktree remove "$WORKTREE_DIR" --force
   ```
   失敗時は警告を出力:
   ```
   ⚠️ Worktree の自動削除に失敗しました。手動で削除してください:
     git worktree remove {path} --force
   ```

#### PRコメントへのレビュー結果投稿

Phase 4 のレビューエージェントの出力をそのまま PR コメントとして投稿する。

**投稿方法:**

```bash
gh pr comment <PR番号> --body "$(cat <<'EOF'
<レビュー結果のMarkdown>
EOF
)"
```

**レビューコメントフォーマット（日本語/英語セクション分離）:**

**ルール**: 1つのPRコメント内で、日本語のレビュー全体を先に出力し、`---` の区切り線の後に英語の同一内容を出力する。各セクション内では単一言語のみ使用する（行単位の併記は行わない）。

````markdown
## 📊 ISAC Autopilot レビュー: XX/100

### 観点別スコア
| 観点 | スコア | 重み | 寄与 |
|------|--------|------|------|
| コード品質 | XX | 30% | XX |
| セキュリティ | XX | 25% | XX |
| テスト充足度 | XX | 20% | XX |
| PR品質 | XX | 15% | XX |
| 整合性 | XX | 10% | XX |

### PR情報
- 変更ファイル数: X
- 追加行数: +XXX
- 削除行数: -XXX

---

## 🔍 詳細レビュー

### 🔒 セキュリティ
| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🔴 高 | `path/to/file` | 日本語の説明 |

### ⚡ パフォーマンス
| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🟡 中 | `path/to/file` | 日本語の説明 |

### 📐 品質・保守性
| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🟢 低 | `path/to/file` | 日本語の説明 |

### 🤔 懐疑的レビュアー
| 重要度 | ファイル | 指摘内容 |
|--------|----------|---------|
| 🟡 中 | `path/to/file` | 日本語の説明 |

---

## 🔧 改善点（+XX）

1. **filename: 指摘タイトル** (+X)
   - 現状: ...
   - 改善案: ...

---

## 📋 PR品質チェック

### 基本
- ✅ PRサイズ: 適切（XXX行）
- ✅ コミット粒度: 適切
- ⚠️ テスト: 機能追加に対するテストがありません

### 整合性
- ✅ インポート/エクスポート: 整合
- ✅ 型定義の一貫性: 統一
- ✅ 命名規則: 統一

---

### 修正ループ経緯

{ループが発生した場合のみ記載}

| Iteration | スコア | 主な指摘 | 修正内容 |
|-----------|--------|---------|---------|
| 1 | XX/100 | {指摘} | {修正} |
| 2 | XX/100 | {指摘} | {修正} |

{ループが発生しなかった場合: 「初回レビューで合格」}

**全て改善した場合の予想スコア: XX/100**

---

## 🌐 English Version

## 📊 ISAC Autopilot Review: XX/100

### Score by Category
| Category | Score | Weight | Contribution |
|------|--------|------|------|
| Code Quality | XX | 30% | XX |
| Security | XX | 25% | XX |
| Test Coverage | XX | 20% | XX |
| PR Quality | XX | 15% | XX |
| Consistency | XX | 10% | XX |

### PR Info
- Changed files: X
- Additions: +XXX
- Deletions: -XXX

---

## 🔍 Detailed Review

### 🔒 Security
| Severity | File | Issue |
|--------|----------|---------|
| 🔴 Critical | `path/to/file` | English description |

### ⚡ Performance
| Severity | File | Issue |
|--------|----------|---------|
| 🟡 Warning | `path/to/file` | English description |

### 📐 Quality & Maintainability
| Severity | File | Issue |
|--------|----------|---------|
| 🟢 Info | `path/to/file` | English description |

### 🤔 Skeptical Reviewer
| Severity | File | Issue |
|--------|----------|---------|
| 🟡 Warning | `path/to/file` | English description |

---

## 🔧 Improvements (+XX)

1. **filename: Issue title** (+X)
   - Current: ...
   - Suggestion: ...

---

## 📋 PR Quality Check

### Basics
- ✅ PR size: Appropriate (XXX lines)
- ✅ Commit granularity: Appropriate
- ⚠️ Tests: No tests for new features

### Consistency
- ✅ Import/Export: Consistent
- ✅ Type definitions: Consistent
- ✅ Naming conventions: Consistent

---

### Iteration History

{Only if iterations occurred}

| Iteration | Score | Key Issues | Fixes |
|-----------|--------|---------|---------|
| 1 | XX/100 | {issues} | {fixes} |
| 2 | XX/100 | {issues} | {fixes} |

{If no iterations: "Passed on first review"}

**Potential score with all improvements: XX/100**

---

> 🤖 Generated by ISAC Autopilot `/isac-autopilot`
````

## 出力フォーマット

進捗を以下の形式で出力してください：

```
🚀 ISAC Autopilot 開始
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 要件: {要件の要約}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🧠 Phase 0: コンテキスト収集
  CLAUDE.md: {読み込み済み / 未検出}
  Memory Service: {接続済み / 未接続（スキップ）}
  MCP ツール: {利用可能なMCPツール一覧 / なし}
  ✓ グローバルルール: {n}件取得
  ✓ プロジェクト記憶: {n}件取得
  ✓ 主要ルール: {ルール要約}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📐 Phase 1: 設計
  Step 1: 影響範囲の特定
    ✓ 変更対象: {ファイル数}ファイル
    ✓ {ファイル一覧}
  Step 2: テストシナリオ
    ✓ {シナリオ数}件（正常系: {n}, 異常系: {n}, 境界値: {n}, エッジケース: {n}）
  Step 3: 4ペルソナ設計レビュー
    ✓ 実装者: {判定}
    ✓ 運用者: {判定}
    ✓ アーキテクト: {判定}
    ✓ 懐疑的レビュアー: {判定}
    ✓ 結論: {設計方針の最終判断}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🌿 Phase 1.5: Worktree 準備
  ✓ ブランチ: {branch}
  ✓ Worktree: {worktree_dir}
  ✓ カレントディレクトリ: {pwd}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔄 Iteration 1/3

💻 Phase 2: 実装
  ✓ {変更内容}

🧪 Phase 3: テスト実行
  ✓ {テスト結果}

🔍 Phase 4: 4ペルソナレビュー（別エージェント）
  ✓ スコア: {score}/100（🔴: {n}件）
  {不合格の場合: → Phase 2 に戻って修正（🔴指摘を優先）}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📝 Phase 5: Draft PR作成 + クリーンアップ
  ✓ ブランチ: {branch}
  ✓ コミット: {message}
  ✓ PRコメント: 4ペルソナレビュー結果投稿完了
  ✓ Worktree: クリーンアップ完了

🎉 完了！

Draft PR: {URL}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
サマリー:
  - 総イテレーション: {count}回
  - 最終レビュースコア: {score}/100
  - テスト結果: {passed/failed}
```

## PR本文フォーマット

Draft PR作成時は以下のフォーマットで本文を作成してください：

```markdown
## 概要

{要件の1-2行要約}

## 設計

### 影響範囲

| ファイル | 変更種別 | 変更内容 | 影響 |
|---------|---------|---------|------|
| {path} | 追加/修正/削除 | {概要} | {依存先への影響} |

### 実装方針

{Phase 1で決定した実装方針を記載}

### テストシナリオ

| # | シナリオ | 種別 | 期待結果 |
|---|---------|------|---------|
| 1 | ... | 正常系 | ... |
| 2 | ... | 異常系 | ... |

### 設計レビュー結果（4ペルソナ）

| ペルソナ | 判定 | コメント |
|---------|------|---------|
| 実装者 | ✅/⚠️ | {コメント} |
| 運用者 | ✅/⚠️ | {コメント} |
| アーキテクト | ✅/⚠️ | {コメント} |
| 懐疑的レビュアー | ✅/⚠️ | {コメント} |

結論: {設計方針の最終判断}

### 技術的な判断

{実装中に行った技術的な判断があれば記載}

## 変更内容

{変更内容の箇条書き}

## レビュー結果

- 最終スコア: {score}/100
- レビューイテレーション: {iteration_count}回
- レビュー方式: 4ペルソナレビュー（別エージェント実行）

{ループが発生しなかった場合: 「初回レビューで合格（90点以上）」}
{ループが発生した場合: 修正ループ経緯テーブル}

## テスト結果

- ステータス: {合格/不合格}
- テストコマンド: `{test_command}`

```
{テスト出力の抜粋（成功/失敗の要約）}
```

---

🤖 This PR was created by [ISAC Autopilot](https://github.com/kita-tech/isac)

Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com>
```

## 重要な制約

- **質問禁止**: ユーザーに質問や確認を求めない
- **自律判断**: 不明点は最善の判断で進める
- **エラー時も続行**: 可能な限り自動で解決を試みる
- **最大2回ループ**: Phase 4 で不合格（90点未満 or 🔴指摘あり）の場合、Phase 2→3→4 を最大2回ループ（3回目の不合格で人間に引き継ぎ）
- **Phase 1 には戻らない**: ループ時は Phase 2（実装）からやり直す

## 失敗時の出力

3回のレビューでも合格しない場合：

```
⚠️ Autopilot: 3回のレビューで合格できませんでした。

現在の状態:
  - 簡易チェック: {通過/未通過}
  - テスト結果: {status}

最後の指摘事項:
  {issues}

変更はブランチ `{branch}` に保存されています。
手動での確認・修正をお願いします。
```

**注意**: 失敗時も Worktree のクリーンアップ（Phase 5 の手順7）を実行すること。PR作成前でも、コミット・push 後にクリーンアップを行う。

---

## ユーザーへの返答テンプレート

Claude は Task tool を起動する前に、以下のように返答してください：

```
🚀 ISAC Autopilot を実行します。

📋 要件: {要件の要約}

完了まで少々お待ちください。
```

完了後、結果（Draft PR URL、スコア等）をユーザーに報告してください。

## 制限事項

- 大規模な変更（10ファイル以上）は非推奨
- セキュリティクリティカルな変更は手動レビューを推奨
- CIが必要なプロジェクトでは、Draft PRマージ前に必ずCI通過を確認

## 関連スキル

- `/isac-code-review` - コードレビュー（単体実行）
- `/isac-pr-review` - PRレビュー（PRコメント投稿）
- `/isac-review` - 設計レビュー
- `/isac-decide` - 決定の記録
- `/isac-memory` - 記憶の検索・管理
- `/isac-save-memory` - AI分析による保存形式提案
- `/isac-suggest` - 状況に応じたSkill提案
